...
Baslinjen är den befintliga implementationen vid `8a7da8d6fb683ec3e92fb2b50aed44d9054a8e1d`, kompletterad med regressionstester i `cff46b8`. Produktionslogiken ändrades inte i checkpointen.

## Strategisk riktning

Aiproxy går från att vara ett utvecklarverktyg för FinOps och SecOps till att vara kärnan i en B2B SaaS-tjänst för Enterprise Compliance och EU AI Act. Den befintliga routinglogiken behålls, men systemets huvudsakliga syfte blir spårbar och integritetsbevarande hantering av företags LLM-trafik.

Arkitekturen delas i två tydliga förtroendedomäner:

1. **Data Plane — befintlig Go-binär.** Körs on-premises eller i kundens moln. Den hanterar LLM-anrop, redigerar PII före vidarebefordran, skriver en AES-256-krypterad rålogg till kundens egen AWS S3 med WORM/Object Lock och skickar endast anonymiserad, kryptografiskt signerad telemetri till Control Plane.
2. **Control Plane — kommande SaaS i Next.js.** Tar emot anonymiserad och signerad telemetri, bygger en Merkle-kedja för oföränderlig bevisföring och genererar compliance-rapporter. Control Plane får aldrig ta emot eller kunna rekonstruera kundens LLM-promptar i klartext.

Kommande implementationer ska fokusera på tre områden: oföränderlig loggning, asynkron telemetri och PII-redigering. Routing och caching får inte ändras om användaren inte uttryckligen begär det i ett specifikt byggsteg. Nästa implementation specificeras i kommande bygg-prompter; den ska inte gissas utifrån den strategiska riktningen.

**Kontext-prompt att ge till nästa kod-LLM**

> Du arbetar vidare i Aiproxy, som nu är kärnan i en B2B SaaS-tjänst för Enterprise Compliance och EU AI Act. Systemet består av ett kundplacerat Data Plane i Go och ett kommande Control Plane i Next.js. Data Plane redigerar PII före LLM-anrop, sparar en AES-256-krypterad rålogg i kundens egen AWS S3 med WORM/Object Lock och skickar endast anonymiserad, kryptografiskt signerad telemetri. Control Plane bygger en Merkle-kedja och genererar compliance-rapporter utan att någonsin se kundens promptar i klartext.
>
> Nuvarande routing, caching och rate-limiting är en etablerad baslinje med regressionstester och en separat checkpoint-commit. Behandla dem som befintliga komponenter att integrera med. Ändra inte routing eller caching om användaren inte uttryckligen begär det i det aktuella byggsteget. All ny funktionalitet ska höra till oföränderlig loggning, asynkron telemetri eller PII-redigering, om inte användaren uttryckligen utökar omfattningen.
>
...
>
> Utöka genom avgränsade komponenter och befintliga gränssnitt. Gör ingen generell omskrivning av proxy, routing, caching, rate-limiting eller konfiguration som en sidoeffekt av ett annat byggsteg. Om en uttryckligen begärd funktion kräver ändring i kärnan, beskriv konkret vilka befintliga beteenden som påverkas och gör minsta nödvändiga ändring med relevanta regressionstester.
>
> Upprätthåll förtroendegränsen mellan planen. Råa promptar, råa svar, PII, krypteringsnycklar och återidentifierande metadata får inte skickas till Control Plane. Telemetri ska vara dataminimerad, anonymiserad, signerad och levereras asynkront så att portalens tillgänglighet inte ligger i Data Planes kritiska anropsväg. Gör inga egna antaganden om nyckelhantering, tenantidentitet, schemastruktur, leveransgarantier eller Merkle-format; invänta respektive bygg-prompt.
>
