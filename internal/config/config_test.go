...
func TestLoad_ParsesMaxRequestsPerMinutePerIP(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "aiproxy.json")
	...
	if cfg.MaxRequestsPerMinutePerIP != 30 {
		t.Fatalf("MaxRequestsPerMinutePerIP = %d, want 30", cfg.MaxRequestsPerMinutePerIP)
	}
}

func TestLoad_MaxRequestsPerMinutePerIPDefaultsToZero(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "aiproxy.json")
	if err := os.WriteFile(path, []byte(`{}`), 0o644); err != nil {
		t.Fatalf("write config: %v", err)
	...
	if cfg.MaxRequestsPerMinutePerIP != 0 {
		t.Fatalf("MaxRequestsPerMinutePerIP = %d, want 0 (disabled) when absent", cfg.MaxRequestsPerMinutePerIP)
	}
}
