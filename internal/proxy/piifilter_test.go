...
func TestPIIFilter_RedactsOnlyForwardedBodyAfterRouting(t *testing.T) {
	...
	requestBody := `{"model":"chat-small","prompt":"Maila anna@example.com, ring 070-123 45 67, använd 900101-0017 eller besök Sankt Eriksgatan 12B"}`
	...
	want := `{"model":"chat-small","prompt":"Maila [REDACTED_EMAIL], ring [REDACTED_PHONE], använd [REDACTED_SSN] eller besök [REDACTED_ADDRESS]"}`
	if forwarded != want {
